...
from core_types import GoalTypes, Transition, Episode
from memories.episodic_experience_replay import EpisodicExperienceReplayParameters, EpisodicExperienceReplay
...
class EpisodicHindsightExperienceReplayParameters(EpisodicExperienceReplayParameters):
    ...
    @property
    def path(self):
        return 'memories.episodic_hindsight_experience_replay:EpisodicHindsightExperienceReplay'
    ...
    def store_episode(self, episode: Episode, lock: bool=True) -> None:
        ...
        if self.action_replay:
            for transition in episode.transitions:
                transition.action = transition.next_state[self.goals_action_space.goal_type]

        # generate hindsight transitions only when an episode is finished
        if self.goal_replay:
            # generate hindsight experience replay goals
            last_episode_transitions = episode.transitions
