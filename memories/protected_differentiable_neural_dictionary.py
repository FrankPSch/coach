...
class AnnoyDictionary(object):
    def __init__(self, dict_size, key_width, new_value_shift_coefficient=0.1, batch_size=1600, key_error_threshold=0.01,
                 ):
        ...
        # in order to use the DND with a scratchpad, we have to use a lock created by a Manager. Otherwise we get an
        # exception that a Lock can only be passed through inheritance. Although we are currently not passing the DND
        # through a scratchpad, we do use this kind of Lock so to be ready once the scratchpad issue is solved.
        self.num_readers_lock = Manager().Lock()
        self.writers_lock = Manager().Lock()
        # self.num_readers_lock = multiprocessing.Lock()
    ...
    def add(self, keys, values):
        ...
        self.release_writing_and_reading()
    ...
    def has_enough_entries(self, k):
        ret_value = self.curr_size > k and (self.built_capacity > k)
        return ret_value
...
class QDND:
    def __init__(self, dict_size, key_width, num_actions, new_value_shift_coefficient=0.1, key_error_threshold=0.01,
                 learning_rate=0.01, num_workers=1):
        ...
        for a in range(num_actions):
            new_dict = AnnoyDictionary(dict_size, key_width, new_value_shift_coefficient,
                                       key_error_threshold=key_error_threshold, batch_size=num_workers * 100)
