...
from base_parameters import iterable_to_items, TaskParameters, DistributedTaskParameters, VisualizationParameters, \
    Parameters, TestingParameters
from core_types import TotalStepsCounter, RunPhase, PlayingStepsType, TrainingSteps, EnvironmentEpisodes, EnvironmentSteps, \
    StepMethod
...
class GraphManager(object):
    ...
    def __init__(self,
                 ...
                 vis_params: VisualizationParameters = VisualizationParameters()):  # TODO: configure framework from outside
        self.sess = None
        self.level_managers = []
        ...
        self.test_params = TestingParameters()
    ...
    def create_session(self, task_parameters: DistributedTaskParameters):
        ...
        if isinstance(task_parameters, DistributedTaskParameters):
            ...
            self.checkpoint_saver = tf.train.Saver(variables_to_restore)
    ...
    def handle_episode_ended(self) -> None:
        ...
        # TODO: this was moved into the layer manager behavior scheme. make sure everything is working before
        # removing this completely
        # [manager.end_episode() for manager in self.level_managers]

        # TODO: this shouldn't be here. it should be called from outside this method. we should reset the internal state
        #       when needed.
    ...
    def restore_checkpoint(self):
        if hasattr(self.task_parameters, 'checkpoint_restore_dir') and self.task_parameters.checkpoint_restore_dir:
            import tensorflow as tf
            checkpoint = tf.train.latest_checkpoint(self.task_parameters.checkpoint_restore_dir)
            screen.log_title("Loading checkpoint: {}".format(checkpoint))
            self.checkpoint_saver.restore(self.sess, checkpoint)

    def save_checkpoint(self):
        if self.task_parameters.save_checkpoint_secs and \
                    time.time() - self.last_checkpoint_saving_time >= self.task_parameters.save_checkpoint_secs:
