...
from architectures.tensorflow_components.architecture import TensorFlowArchitecture
from configurations import AgentParameters, InputEmbedderParameters
from core_types import PredictionType
from spaces import SpacesDefinition, PlanarMapsObservationSpace
import tensorflow as tf
from utils import short_dynamic_import, get_all_subclasses, dynamic_import_and_instantiate_module_from_params
...
class GeneralTensorFlowNetwork(TensorFlowArchitecture):
    ...
    def __init__(self, agent_parameters: AgentParameters, spaces: SpacesDefinition, name: str,
        ...
        self.num_heads_per_network = 1 if self.network_parameters.use_separate_networks_per_head else \
            len(self.network_parameters.output_types)
        ...
        self.activation_function = self.get_activation_function(
            self.network_parameters.hidden_layers_activation_function)
        self.middleware_hidden_layer_size = self.network_parameters.middleware_hidden_layer_size
        ...
        def fill_return_types():
            ...
            for cls in get_all_subclasses(PredictionType):
                ret_dict[cls] = []
            components = self.input_embedders + [self.middleware_embedder] + self.output_heads
    ...
    def get_input_embedder(self, input_name: str, embedder_params: InputEmbedderParameters):
        ...
        """
        available_inputs = copy.copy(self.spaces.state.sub_spaces)
        available_inputs["action"] = copy.copy(self.spaces.action)
        available_inputs["goal"] = copy.copy(self.spaces.goal)

        if input_name not in available_inputs.keys():
            raise ValueError("The key for the input embedder ({}) must match one of the following keys: {}"
                             .format(input_name, available_inputs.keys()))

        type = "vector"
        if isinstance(available_inputs[input_name], PlanarMapsObservationSpace):
            type = "image"

        embedder_path = 'architectures.tensorflow_components.embedders.' + embedder_params.path[type]
        ...
        module = dynamic_import_and_instantiate_module_from_params(embedder_params_copy,
                                                                   path=embedder_path,
                                                                   positional_args=[available_inputs[input_name].shape])
        return module

    def get_middleware_embedder(self, middleware_type: str):
        """
        Given a middleware type, creates the middleware and returns it
        :param middleware_type: the path to the class of the middleware under the middlewares directory or a full path
                                to a middleware class.
                                the path should be in the following structure: <module_path>:<class_path>
        :return: the middleware instance
        """
        # TODO: allow external middleware types
        middleware_type = 'architectures.tensorflow_components.middlewares.' + middleware_type
        return short_dynamic_import(middleware_type)(self.activation_function, self.middleware_hidden_layer_size)

    def get_output_head(self, head_type: str, head_idx: int, loss_weight: float=1.):
        """
        ...
        :return: the head
        """
        # TODO: allow external head types
        head_type = 'architectures.tensorflow_components.heads.' + head_type
        return short_dynamic_import(head_type)(
    ...
    def get_model(self):
        ...
        if len(self.network_parameters.input_types) == 0:
            raise ValueError("At least one input type should be defined")

        if len(self.network_parameters.output_types) == 0:
            raise ValueError("At least one output type should be defined")

        if self.network_parameters.middleware_type is None:
        ...
        if len(self.network_parameters.output_types) != len(self.network_parameters.loss_weights):
        ...
        for network_idx in range(self.num_networks):
            with tf.variable_scope('network_{}'.format(network_idx)):
                ...
                state_embedding = []
                for input_name, input_type in self.network_parameters.input_types.items():
                ...
                self.middleware_embedder = self.get_middleware_embedder(self.network_parameters.middleware_type)
                ...
                for head_idx in range(self.num_heads_per_network):
                    for head_copy_idx in range(self.network_parameters.num_output_head_copies):
                        ...
                        self.output_heads.append(
                            self.get_output_head(self.network_parameters.output_types[head_type_idx],
                                                 head_copy_idx,
                        ...
                        self.gradients_from_head_rescalers_placeholders.append(
                            tf.placeholder('float',
                                           name='gradients_from_head_{}-{}_rescalers'.format(head_idx, head_copy_idx)))
        ...
        if local_network_in_distributed_training and \
                hasattr(self.network_parameters, "shared_optimizer") and self.network_parameters.shared_optimizer:
            # distributed training and this is the local network instantiation
            self.optimizer = self.global_network.optimizer
        else:
            self.current_learning_rate = self.adaptive_learning_rate_scheme if self.adaptive_learning_rate_scheme is \
                                    not None else self.network_parameters.learning_rate

            if self.network_parameters.optimizer_type == 'Adam':
                self.optimizer = tf.train.AdamOptimizer(learning_rate=self.current_learning_rate, beta1=0.9, beta2=0.99,
                                                        epsilon=self.network_parameters.optimizer_epsilon)  # openai params
            elif self.network_parameters.optimizer_type == 'RMSProp':
                self.optimizer = tf.train.RMSPropOptimizer(self.current_learning_rate, decay=0.9,
                                                           epsilon=self.network_parameters.optimizer_epsilon)  # TODO: optimizer epsilon for RMSProp was originally 0.01
