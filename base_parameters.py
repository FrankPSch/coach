...
from core_types import TrainingSteps, EnvironmentSteps, GradientClippingMethod
from filters.filter import NoInputFilter
...
class TestingParameters(Parameters):
    def __init__(self):
        super().__init__()
...
class InputEmbedderParameters(Parameters):
    def __init__(self, activation_function: str='relu', scheme: Union[List, EmbedderScheme]=EmbedderScheme.Medium,
                 batchnorm: bool=False, dropout=False, name: str='embedder', input_normalization=None,
                 input_clipping=None):
        super().__init__()
        if input_normalization is None:
            input_normalization = {'image': (255.0, 0.0), 'vector': (1.0, 0.0)}
