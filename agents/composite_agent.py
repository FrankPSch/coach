...
class MajorityVote(DecisionPolicy):
    ...
    def choose_action(self, actions_info: Dict[str, ActionInfo]) -> ActionInfo:
        ...
        :return: a single action
        """
        # TODO: this is only relevant for discrete action spaces
...
class MeanDecision(DecisionPolicy):
    ...
    def choose_action(self, actions_info: Dict[str, ActionInfo]) -> ActionInfo:
        ...
        :return: a single action
        """
        # TODO: this is only relevant for continuous action spaces
        if len(actions_info.keys()) == 0:
            raise ValueError("The given list of actions is empty")
        mean = np.mean([action_info.action for action_info in actions_info.values()], axis=0)
        return ActionInfo(mean)  # TODO: what is the action info to fill?
...
class CompositeAgent(AgentInterface):
    ...
    def __init__(self,
                 ...
                 raw_observation_space: ObservationSpace,
        ...
        :param raw_observation_space: the observation space of the real environment
        ...
        self.raw_observation_space = raw_observation_space  # TODO: this is not being used
    ...
    def act(self) -> ActionInfo:
        ...
        # TODO: why not pass the action itself?
    ...
    def observe(self, env_response: EnvResponse) -> bool:
        ...
        for agent_name, agent in self.agents.items():
            done = agent.observe(env_response)
            episode_ended = episode_ended or done
